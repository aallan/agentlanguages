...
camp: syntactic
spans_camps: []
one_liner: "Designed for AI to write, with one canonical way to express every operation. Nine bundled modules, structured JSON errors with fix/retry fields, ? operator for propagation. Compiles to Go IR, then native via go build."
url: https://codong.org
repo: brettinhere/Codong
paper: null
author: Brett (brettinhere)
implementation_language: Go
compilation_target: Native binary via Go IR + `go build`
license: MIT
