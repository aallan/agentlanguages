...
camp: syntactic
spans_camps: []
one_liner: "A Jason Hall blog post on three attempts at an LLM-optimised language: B-IR with unicode opcodes, TBIR with control characters the model rewrote into English keywords, and Loom with pre/postconditions and stable error codes."
url: https://articles.imjasonh.com/llm-programming-language.md
repo: imjasonh/loom
paper: null
author: Jason Hall (Chainguard)
implementation_language: Python (bootstrap)
compilation_target: Arm64 assembly (Mach-O via clang)
license: Unknown
...
crossrefs:
  - slug: sever
    name: Sever
    camp: syntactic
    relation: "Both are catalogue-meta companions. Each captures what falls out when a frontier model is asked to design a language for itself; both authors keep the result at arm's length from any production claim."
  - slug: laze
    name: Laze
    camp: syntactic
    relation: "Both are small explorations by a single author working with one model over a weekend. Laze ships a compiler; B-IR ships an article."
  - slug: vera
    name: Vera
    camp: verification
    relation: "Loom's conclusions — unambiguous scope, mandatory pre/postconditions, stable error codes — converge on the diagnosis Vera arrived at independently in the verification camp."
---
